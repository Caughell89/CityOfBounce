...
import com.example.springsocial.repository.ProductFeatureRepository;
import com.example.springsocial.repository.ProductRepository;
import com.example.springsocial.repository.ProductReqRepository;
...
public class SearchController {
    ...
    @Autowired
    private ProductReqRepository productReqRepository;
    ...
    public SearchResult getProductsBySearchFilters(@RequestBody SearchRequest searchRequest) {
        if (searchRequest.getDate().equalsIgnoreCase("Any")) {
            searchRequest.setDate("1900-01-01");
        } else {
            System.out.println("Find them for " + searchRequest.getDate());
        }
        ...
        if (searchRequest.getSortBy().equalsIgnoreCase("Newest")) {
            foundProducts = productRepository.findProductsBySearchFiltersSortedByCreatedOn(searchRequest.getProductTypes(), city, state, searchRequest.isInstantBook(),
                    ...
                    searchRequest.getDate(), searchRequest.getRow(), 20);
    ...
    public Product getProductById(@PathVariable Long productId) {
        ...
        foundProduct.setProductFeatures(productFeatureRepository.findByProductId(foundProduct.getProductId()));
        foundProduct.setProductReqs(productReqRepository.findByProductId(foundProduct.getProductId()));
